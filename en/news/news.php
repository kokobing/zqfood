...
$intRows = 10;
$strSQLNum = "SELECT COUNT(*) as num from newsinfo as a left join newsdir as b on a.id_newsdir=b.id_newsdir where a.dele=1 and b.lang=0 ";   
...
$strSQL = "select a.*,b.fatherid from newsinfo as a left join newsdir as b on a.id_newsdir=b.id_newsdir where a.dele=1 and b.lang=0 order by a.id_newsinfo desc" ;
...
//新闻二级目录
$strSQL = "select name,id_newsdir from newsdir where lang=0 and level=2 order by id_newsdir asc " ;
$objDB->Execute($strSQL);
$ndir2=$objDB->getrows();
...
<ul class="sub_nav">
            ...
            <? for($i=0;$i<sizeof($ndir2);$i++){?>
				<li><a href="/en/news/news.php?ndir2=<?=$ndir2[$i][id_newsdir]?>" ><?=$ndir2[$i][name]?></a> </li>
			<? }?>	 
		  </ul>
</div> <!--end about_cleft!-->
<div id="about_cright">
...
Now：<a href="/" class="link_aboutnavi">Home</a> > <a href="/en/news/news.php" class="link_aboutnavi">News</a> > <?=$newsdirnavi[name];?></div> 
<div id="about_crighttitle">&nbsp;&nbsp;&nbsp;&nbsp;News</div> 
<div class="txt" id="newscontentright_content">

<div id="allnews">
  <ul>
  <?php for($i=0;$i<sizeof($arrnews);$i++){?>
    <li><span><?=$arrnews[$i][newsdate];?></span><a href="newsinfo.php?newsid=<?=$arrnews[$i][id_newsinfo];?>&ndir2=<?=$_GET[ndir2]?>" class="link_navi"><?=cutstr($arrnews[$i][title],70,1);?></a></li>
  <?php }?>
  </ul>
</div><!--end allnews!-->
