...
class PageNav{
	...
	*/
	function GetLink(){
		$strLink="Number".$this->mTotalRecord."&nbsp;Total".$this->mTotalPage."&nbsp;Current Page".$this->mCurPage."
	...
	function init_page ( $pagesize, $num, $page=false, // 通常指定前三项就 OK
		...
		for($i = max ( 1, $page - $links ); $i <= min ( $this->tpage, $page + $links ); $i++){
			if($page == $i){
				...
				$PageLinks .= "<a href=\"{$this->file}?{$this->varstr}{$f}$var=$i\" class=\"link_navi_1\">$i</a>&nbsp;&nbsp;"; // 其他页的链接
			}
		}
		...
		if ( $this->tpage > 0 ){
			$before	= $page > 1 // 若当前页大于 1， 显示“第一页”
				? "<a href=\"{$this->file}?{$this->varstr}{$f}$var=1\" class=\"link_navi_1\">$txt_fp</a>"
				: "";
			$after	= $page < $this->tpage // 若当前页小于最后页， 显示“最后页”
				? "<a href=\"{$this->file}?{$this->varstr}{$f}$var={$this->tpage}\" class=\"link_navi_1\">$txt_lp</a>"
				: "";

			
				if ( $page > 1 ){ // 若当前页大于 1， 显示“上一页”
					$before	.= " <a href=\"{$this->file}?{$this->varstr}{$f}$var=" . ( $page - 1 ) . "\" class=\"link_navi_1\">$txt_pp</a>";
				}
				if ( $page < $this->tpage ){ // 若当前页小于最后页， 显示“下一页”
					$after	= "<a href=\"{$this->file}?{$this->varstr}{$f}$var=" . ( $page + 1 ) . "\" class=\"link_navi_1\">$txt_np</a> " . $after;
				}
