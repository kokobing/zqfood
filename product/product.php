...
$strSQL = "select a.title,a.content,a.id_prodinfo,a.id_proddir,b.opicname,c.fatherid from productinfo as a 
          left join productpic as b on a.id_prodinfo=b.id_prodinfo 
		  left join productdir as c on a.id_proddir=c.id_proddir
		  where a.dele='1'  and c.lang='1' order by a.id_prodinfo desc" ;
...
$strSQL = "select a.title,a.content,a.id_prodinfo,a.id_proddir,b.opicname,c.fatherid from productinfo as a 
           left join productpic as b on a.id_prodinfo=b.id_prodinfo
		   left join productdir as c on a.id_proddir=c.id_proddir
		   where c.fatherid='".$_GET[pdir1]."' and a.dele=1 and c.lang='1' order by a.id_prodinfo desc" ;
$objDB->SelectLimit($strSQL,$intRows,$intStartNum);
$arrprods=$objDB->GetRows();
  ...
  $arrParam[1][value]=$_GET[pdir2];
  
$intRows = 9;
$strSQLNum = "SELECT COUNT(*) as num from productinfo as a 
           left join productpic as b on a.id_prodinfo=b.id_prodinfo
		   left join productdir as c on a.id_proddir=c.id_proddir
		   where c.id_proddir='".$_GET[pdir2]."' and a.dele=1 and c.lang='1'";   
$rs = $objDB->Execute($strSQLNum);
$arrNum = $objDB->fields();
$intTotalNum=$arrNum["num"];
...
//产品一级目录
$strSQL = "select name,id_proddir from productdir where lang=1 and level=1 order by id_proddir asc " ;
...
<ul class="sub_nav">
            ...
            <? for($i=0;$i<sizeof($pdir1);$i++){?>
				<li><a href="/product/product.php?pdir1=<?=$pdir1[$i][id_proddir]?>" ><?=$pdir1[$i][name]?></a> </li>
			<? }?>	 
		  </ul>
</div> <!--end about_cleft!-->
<div id="about_cright">
